...
function tabs() {
    ...
    if (mdQueriesArray && mdQueriesArray.length) {
      mdQueriesArray.forEach((mdQueriesItem) => {
        mdQueriesItem.matchMedia.addEventListener("change", function() {
          ...
          reinitShowMoreAfterDelay();
        });
        setTitlePosition(mdQueriesItem.itemsArray, mdQueriesItem.matchMedia);
      });
    }
  }
  function reinitShowMoreAfterDelay(delay = 100) {
    setTimeout(() => {
      if (window.reinitShowMore) {
        window.reinitShowMore();
      }
    }, delay);
  }
  ...
  function setTabsStatus(tabsBlock) {
    ...
    if (tabsContent.length > 0) {
      ...
      const totalAnimations = tabsContent.length;
      tabsContent.forEach((tabsContentItem, index) => {
        if (tabsTitles[index].classList.contains("--tab-active")) {
          if (tabsBlockAnimate) {
            ...
            setTimeout(() => {
              animationCompleted++;
              if (animationCompleted === totalAnimations) {
                reinitShowMoreAfterDelay(50);
              }
            }, tabsBlockAnimate);
          } else {
...
function showMore() {
  ...
  let mdQueriesArray;
  function reinitShowMore() {
    console.log("reinitShowMore called");
    const currentShowMoreBlocks = document.querySelectorAll("[data-fls-showmore]");
    showMoreBlocksRegular = Array.from(currentShowMoreBlocks).filter(function(item) {
      return !item.dataset.flsShowmoreMedia;
    });
    if (showMoreBlocksRegular.length) {
      initItems(showMoreBlocksRegular);
    }
    const currentMdQueriesArray = dataMediaQueries(currentShowMoreBlocks, "flsShowmoreMedia");
    if (currentMdQueriesArray && currentMdQueriesArray.length) {
      currentMdQueriesArray.forEach((mdQueriesItem) => {
        if (mdQueriesItem.itemsArray.length) {
          initItems(mdQueriesItem.itemsArray, mdQueriesItem.matchMedia);
        }
      });
    }
  }
  window.reinitShowMore = reinitShowMore;
  ...
  function initItems(showMoreBlocks2, matchMedia) {
    showMoreBlocks2.forEach((showMoreBlock) => {
      waitForVisible(showMoreBlock, () => {
        initItem(showMoreBlock, matchMedia);
      });
    });
  }
  function waitForVisible(element, callback, maxAttempts = 10) {
    let attempts = 0;
    function checkVisibility() {
      ...
      if (isVisible && hasSize) {
        ...
        console.log("Element not visible after max attempts:", element);
        callback();
      }
    }
    checkVisibility();
  ...
  function initItem(showMoreBlock, matchMedia = false) {
    if (!showMoreBlock || !document.body.contains(showMoreBlock)) return;
    ...
    let limitValue = showMoreContent.dataset.flsShowmoreContent ? parseInt(showMoreContent.dataset.flsShowmoreContent) : showMoreType === "items" ? 3 : 150;
    let totalCount = 0;
    let needShowMore = false;
    if (showMoreType === "items") {
      const items = Array.from(showMoreContent.children).filter((child) => {
        return child.nodeType === 1;
      });
      totalCount = items.length;
      needShowMore = totalCount > limitValue;
      console.log(`Tab: ${showMoreBlock.closest(".tabs__body")?.parentNode?.querySelector(".--tab-active")?.textContent || "unknown"}, Items: ${totalCount}, Limit: ${limitValue}, Need button: ${needShowMore}`);
    } else {
      const originalHeight = getOriginalHeight(showMoreContent);
      needShowMore = originalHeight > limitValue;
      console.log(`Height: ${originalHeight}, Limit: ${limitValue}, Need button: ${needShowMore}`);
    }
    if (!needShowMore) {
    ...
    const shouldCollapse = matchMedia && matchMedia.matches || !matchMedia;
    if (shouldCollapse) {
  ...
  function getHeight(showMoreBlock, showMoreContent) {
    ...
    if (showMoreType === "items") {
      const limitValue = showMoreContent.dataset.flsShowmoreContent ? parseInt(showMoreContent.dataset.flsShowmoreContent) : 3;
      const children = Array.from(showMoreContent.children).filter((child) => child.nodeType === 1);
      const itemsToShow = Math.min(limitValue, children.length);
      for (let i = 0; i < itemsToShow; i++) {
        ...
        hiddenHeight += childHeight + marginTop + marginBottom;
      }
      if (rowGap && itemsToShow > 1) {
        hiddenHeight += (itemsToShow - 1) * rowGap;
      }
    } else {
      const limitValue = showMoreContent.dataset.flsShowmoreContent ? parseInt(showMoreContent.dataset.flsShowmoreContent) : 150;
      hiddenHeight = limitValue;
    }
    return hiddenHeight;
  }
  function getOriginalHeight(showMoreContent) {
    const currentHeight = showMoreContent.style.height;
    const currentOverflow = showMoreContent.style.overflow;
    showMoreContent.style.height = "auto";
    showMoreContent.style.overflow = "visible";
    const originalHeight = showMoreContent.offsetHeight;
    showMoreContent.style.height = currentHeight;
    showMoreContent.style.overflow = currentOverflow;
  ...
  function showMoreActions(e) {
    ...
    if (targetType === "click") {
      if (targetEvent.closest("[data-fls-showmore-button]")) {
        ...
        const showMoreSpeed = showMoreBlock.dataset.flsShowmoreButton || "500";
        const originalHeight = getOriginalHeight(showMoreContent);
        const hiddenHeight = getHeight(showMoreBlock, showMoreContent);
        if (showMoreBlock.classList.contains("--showmore-active")) {
          showMoreContent.style.transition = `height ${showMoreSpeed}ms ease`;
          showMoreContent.style.height = `${hiddenHeight}px`;
          showMoreContent.style.overflow = "hidden";
          setTimeout(() => {
            showMoreContent.style.transition = "";
          }, showMoreSpeed);
          showMoreBlock.classList.remove("--showmore-active");
        } else {
          showMoreContent.style.transition = `height ${showMoreSpeed}ms ease`;
...
class BeforeAfter {
  ...
  beforeAfterClasses(beforeAfter) {
    if (!isMobile.any()) {
      beforeAfter.addEventListener("pointermove", (e) => {
        if (beforeAfter.dataset.dragging === "true") return;
        ...
        if (!targetElement.hasAttribute("data-fls-beforeafter-arrow")) {
          ...
          } else if (targetElement.closest("[data-fls-beforeafter-after]")) {
            beforeAfter.classList.add("-right");
            beforeAfter.classList.remove("-left");
          }
        }
      });
      beforeAfter.addEventListener("pointerleave", () => {
        beforeAfter.classList.remove("-left", "-right");
      });
    }
  }
  beforeAfterItemInit(beforeAfter) {
    ...
    const startDrag = (e) => {
      ...
      isDragging = true;
      beforeAfter.dataset.dragging = "true";
      beforeAfterSizes = {
        width: beforeAfter.offsetWidth,
        left: beforeAfter.getBoundingClientRect().left
      };
      document.body.style.userSelect = "none";
      document.body.style.webkitUserSelect = "none";
      document.body.style.touchAction = "none";
      beforeAfterArrow.style.cursor = "grabbing";
      beforeAfterArrow.classList.add("_grabbing");
    };
    const moveDrag = (e) => {
      ...
      animationFrame = requestAnimationFrame(() => {
        ...
        const posLeft = clientX - beforeAfterSizes.left;
        let way;
        if (posLeft <= 0) {
          way = 0;
          ...
          way = posLeft / beforeAfterSizes.width * 100;
        }
        const arrowLeft = `calc(${way}% - ${beforeAfterArrowWidth / 2}px)`;
        const afterWidth = `${100 - way}%`;
        beforeAfterArrow.style.left = arrowLeft;
        afterItem.style.width = afterWidth;
      });
    };
    const endDrag = (e) => {
      if (!isDragging) return;
      isDragging = false;
      beforeAfter.dataset.dragging = "false";
      if (animationFrame) {
        cancelAnimationFrame(animationFrame);
        animationFrame = null;
      }
      ...
      beforeAfterArrow.style.cursor = "grab";
      beforeAfterArrow.classList.remove("_grabbing");
    };
    if (isMobile.any()) {
      beforeAfterArrow.addEventListener("touchstart", startDrag, { passive: false });
      beforeAfterArrow.addEventListener("touchmove", moveDrag, { passive: false });
      beforeAfterArrow.addEventListener("touchend", endDrag);
      beforeAfterArrow.addEventListener("touchcancel", endDrag);
      document.addEventListener("touchmove", (e) => {
        if (isDragging && e.target.closest("[data-fls-beforeafter]")) {
          e.preventDefault();
        }
      }, { passive: false });
      ...
      beforeAfterArrow.addEventListener("mousedown", startDrag);
      document.addEventListener("mousemove", moveDrag);
      document.addEventListener("mouseup", endDrag);
    }
    beforeAfter.addEventListener("dragstart", (e) => e.preventDefault());
    const handleResize = () => {
      if (isDragging) {
        beforeAfterSizes = {
          width: beforeAfter.offsetWidth,
    ...
    beforeAfter._destroy = () => {
      if (isMobile.any()) {
        beforeAfterArrow.removeEventListener("touchstart", startDrag);
        beforeAfterArrow.removeEventListener("touchmove", moveDrag);
        beforeAfterArrow.removeEventListener("touchend", endDrag);
        beforeAfterArrow.removeEventListener("touchcancel", endDrag);
        document.removeEventListener("touchend", endDrag);
        document.removeEventListener("touchcancel", endDrag);
      } else {
        beforeAfterArrow.removeEventListener("mousedown", startDrag);
